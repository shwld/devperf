...
pub async fn calculate_metrics(owner: &str, repo: &str, since: DateTime<Utc>, until: DateTime<Utc>, developers: u64, working_days_per_week: f32, environment: &str) -> Result<DeploymentMetrics, octocrab::Error> {
    let deployments = github::deployment::list(owner, repo, environment).await.expect("Could not get deployments");
