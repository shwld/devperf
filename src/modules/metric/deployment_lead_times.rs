...
pub async fn calculate(owner: &str, repo: &str, deployment_nodes: Vec<DeploymentItem>, first_committed_at: DateTime<Utc>) -> Result<Vec<DeploymentMetricItem>, octocrab::Error> {
    ...
    for deployment in sorted {
        let first_committed_at = if previous_commit_sha.is_empty() {
            ...
            let first_committed_at_from_compare = compare::get_first_commit_committer_date(owner, repo, &previous_commit_sha, &deployment.commit_sha).await.expect("Could not get first commit committer date");
        ...
        let deployment_metric = DeploymentMetricItem {
            ...
            head_commit: DeploymentCommitItem {
                ...
                message: deployment.commit_message.clone(),
                resource_path: deployment.commit_resource_path.clone(),
                committed_at: deployment.committed_at,
        ...
        previous_commit_sha = deployment.commit_sha.clone();
        previous_commit_message = deployment.commit_message.clone();
        previous_commit_resource_path = deployment.commit_resource_path.clone();
        previous_commit_committed_at = deployment.committed_at.clone();
