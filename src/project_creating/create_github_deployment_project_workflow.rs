...
pub async fn perform<T: WriteNewConfig>(
    write_new_config: T,
    project: UncreatedGitHubDeploymentProject,
) -> Result<CreateGithubDeploymentProjectEvent, CreateGithubDeploymentProjectError> {
    ...
    let project_dto =
        GitHubDeploymentProjectCreatedDto::from_git_hub_deployment_project_created(project.clone());
    write_new_config
        .perform(project_dto)
        .await
        .map_err(CreateGithubDeploymentProjectError::WriteNewConfigError)?;
    Ok(project)
}
