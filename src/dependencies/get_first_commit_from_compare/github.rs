...
pub async fn get_first_commit_from_compare(github_api_client: &GitHubAPIClient, params: &FirstCommitFromCompareParams) -> Result<FirstCommitItem, GetFirstCommitFromCompareError> {
    ...
    if params.base == params.head {
        return Err(GetFirstCommitFromCompareError::BaseEqualsHead(format!("base: {:?}, head: {:?}", params.base, params.head)))
    }
    let path = format!("https://api.github.com/repos/{owner}/{repo}/compare/{base}...{head}", owner = params.owner, repo = params.repo, base = params.base, head = params.head);
    let result = github_api_client._get(path, None::<&()>).await.map_err(|e| anyhow::anyhow!(e)).map_err(GetFirstCommitFromCompareError::APIClientError)?;
    let status = result.status();
    if result.status().is_success() == false {
        return Err(GetFirstCommitFromCompareError::APIResponseError(format!("status: {:?}", status)))
    }
