...
use super::interface::{
    GitHubDeploymentResourceConfig, HerokuReleaseResourceConfig, ProjectConfig, ReadProjectConfig,
    ReadProjectConfigError, ResourceConfig,
};
use crate::dependencies::settings_toml::{Config, ProjectName};
            ...
            .and_then(|c| {
                ...
                if let Some(project_config) = project_config {
                    ...
                    match project_config.clone().deployment_source.as_str() {
                        ...
                        "heroku_release" => Ok(ProjectConfig {
                            project_name,
                            developer_count: project_config.clone().developer_count,
                            working_days_per_week: project_config.clone().working_days_per_week,
                            resource: ResourceConfig::HerokuRelease(HerokuReleaseResourceConfig {
                                github_personal_token: project_config
                                    ...
                                    .github_personal_token
                                    .unwrap_or(c.github_personal_token.clone()),
                                heroku_app_name: project_config.clone().heroku_app_name.ok_or(
                                    ReadProjectConfigError::CannotReadHerokuAppName(
                                        "Cannot read".to_string(),
                                    ),
                                )?,
                                heroku_api_token: project_config.clone().heroku_api_token.ok_or(
                                    ReadProjectConfigError::CannotReadHerokuApiToken(
                                        "Cannot read".to_string(),
                                    ),
                                )?,
                                github_owner: project_config.clone().github_owner,
